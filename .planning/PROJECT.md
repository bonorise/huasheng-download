...
这是一个本地自动化下载工具，用于从华声视频制作 agent 的项目页面批量下载用户筛选过的视频素材。用户输入一个项目 URL，工具复用已登录状态，默认打开全局“收藏”素材 tab，提取收藏素材播放弹窗中的视频地址并保存到桌面 `hs-src` 文件夹；也可以切换到“推荐”模式按分镜下载推荐素材。

## Core Value

稳定、完整地把用户收藏的视频素材下载到本地；需要时仍可按分镜下载推荐素材。
...
- [ ] 工具默认可以打开全局“收藏”tab，下载用户手动收藏的素材。
- [ ] 收藏模式不需要按分镜循环，文件命名使用 `素材01.mp4` 格式。
- [ ] 工具可以通过 `--tab 推荐` 切换为按分镜推荐素材下载模式。
...
| 默认下载全局收藏素材 | 用户发现收藏 tab 跨分镜聚合，数量更可控 | — Pending |
| 推荐素材作为备选模式 | 用户仍可能需要按分镜抽样推荐素材 | — Pending |
| 按分镜重复保存同一素材 | 用户希望每个分镜独立管理 | — Pending |
| 命名格式为 `分镜01_素材01.mp4` | 用户选择最简编号命名 | — Pending |
| 收藏模式命名格式为 `素材01.mp4` | 收藏素材不归属于单个分镜 | — Pending |
