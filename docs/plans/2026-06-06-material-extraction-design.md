...
## 2026-06-06 策略更新：收藏模式

测试后确认“收藏”tab 是跨分镜的全局素材池，不归属于某一个分镜。为了控制素材数量和下载时间，默认下载模式改为“收藏”：

1. 打开项目 URL。
2. 打开素材窗口并点击“展开更多”。
3. 点击“收藏”tab。
4. 在收藏素材容器中循环执行“点击封面 -> 获取 URL -> 点击右上角关闭按钮 -> 下一个”。
5. 捕获完收藏列表后统一下载。
...
“推荐”保留为备选模式。只有显式传入 `--tab 推荐` 时，脚本才按分镜循环处理推荐素材，并继续使用 `分镜01_素材01.mp4` 命名。
